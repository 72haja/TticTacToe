...
export function OuterGameField(props: ItemProps) {
  ...
  useEffect(() => {
    ...
    if (props.player2 === "") {
      if (gameReady) {
        props.setSnackbar({
          color: "error",
          show: true,
          text: "Spieler 2 hat das Spiel verlassen",
          id: uuid(),
          timeout: 3000,
        });
      }
    ...
    props.setSnackbar({
      color: "success",
      show: true,
      text: "Spieler 2 ist beigetreten",
      id: uuid(),
      timeout: 3000,
    });
  }, [props.player2]);
  ...
  const [isMobileOrTablet, setIsMobileOrTablet]: [boolean, (isMobileOrTablet: boolean) => void] = useState(false);
  function mobileAndTabletCheck() {
    let check = false;
    if(!global.navigator) return check;
    (function(a){
            ...
            : <div>
                ...
                </p>
                {
                  isMobileOrTablet && 
                    <button
                      onClick={shareUrl}
                      className="bg-transparent rounded-lg p-1"
                    >
                      <ShareIcon
                        className="w-5 h-5 aspect-square text-white"
                      />
                    </button>
                }
              </div>
